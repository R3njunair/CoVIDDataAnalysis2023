...
var file_cases = "https://raw.githubusercontent.com/R3njunair/CoVIDDataAnalysis2023/main/ModifiedCSV/mortality_timeseries_hr.csv";
var file_mortality = "https://raw.githubusercontent.com/R3njunair/CoVIDDataAnalysis2023/main/ModifiedCSV/mortality_timeseries_hr.csv";
var file_update_time = "https://raw.githubusercontent.com/R3njunair/CoVIDDataAnalysis2023/main/ModifiedCSV/update_time.txt";
var file_hr_lookup = "https://raw.githubusercontent.com/R3njunair/CoVIDDataAnalysis2023/main/ModifiedCSV/health_regions_lookup.csv";
...
Promise.all([
    ...
    var caseNewWorld = cases.reduce((a, b) => +a + +b.case_new_count, 0);
    var mortNewWorld = mortalities.reduce((a, b) => +a + +b.mort_new_count, 0);
    ...
    const caseByCountryLookup = equijoinWithDefault(
        regionLookup, caseByCountry, 
    ...
    const mortByCountryLookup = equijoinWithDefault(
        regionLookup, mortByCountry, 
    ...
    const casemortByCountry = equijoinWithDefault(
        caseByCountryLookup, mortByCountryLookup, 
    ...
    const covidData = equijoinWithDefault(
        regionLookup, casemortByCountry, 
        "province_health_region", "case_prov_health_region", 
    ...
    // get case and mort date max dates to get new counts
    maxcaseByCountry = d3.max(covidData.map(d=>d.case_count));
    maxmortByCountry = d3.max(covidData.map(d=>d.mort_count));
    maxNewcaseByCountry = d3.max(covidData.map(d=>d.case_new_count));
    maxNewmortByCountry = d3.max(covidData.map(d=>d.mort_new_count));
...
//CREATE MAP=================================
    ...
    function createCharts(statscanRegion) {
        ...
        var caseNewPctCanada = parseFloat(regionCaseNewCount / caseNewWorld * 100).toFixed(2)+"%";
        var mortNewPctCanada = parseFloat(regionMortNewCount / mortNewWorld * 100).toFixed(2)+"%";
...
//CREATE TABLE BELOW MAP=================================
    $(document).ready(function () {
        ...
        for(var i = 0; i < covidData.length; i++) {
            ...
            tbody_tr.append("<td style='text-align: right';>" + parseFloat(obj.case_new_count / caseNewWorld * 100).toFixed(2) + "</td>");
            tbody_tr.append("<td style='text-align: right';>" + parseFloat(obj.mort_new_count / mortNewWorld * 100).toFixed(2) + "</td>");
        }
    });
